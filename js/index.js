...
if (
  localStorage.getItem("empolyeesList") == null ||
  JSON.parse(localStorage.getItem("empolyeesList")).length == 0
) {
  empolyeesCont = [];
...
addBtn.onclick = function () {
  if (addBtn.innerHTML == "add Empolyee") addEmp();
  else updateEmp(currentIndex);
...
function displayDataInTable() {
  ...
  for (var i = 0; i < empolyeesCont.length; i++) {
    ...
    <tr>
      <td>` +
      empolyeesCont[i].empolyeeName +
      `</td>
      <td>` +
      empolyeesCont[i].empolyeeAge +
      `</td>
      <td>` +
      empolyeesCont[i].empolyeePhone +
      `</td>
      <td>` +
      empolyeesCont[i].empolyeeTitle +
      `</td>
      <td>
      <button onclick='getCurrentEmp(` +
      i +
...
function deleteEmp(index) {
  ...
  localStorage.setItem("empolyeesList", JSON.stringify(empolyeesCont));
  if (row.classList.contains("hide-grid")) displayDataInTable();
  else displayInGrid();
}
...
function search(term) {
  if (row.classList.contains("hide-grid")) {
    ...
    for (var i = 0; i < empolyeesCont.length; i++) {
      if (
        empolyeesCont[i].empolyeeName.toLowerCase().includes(term.toLowerCase())
      ) {
        trs +=
          "<tr><td>" +
          empolyeesCont[i].empolyeeName +
          ...
          "</td><td>" +
          empolyeesCont[i].empolyeeTitle +
          "</td><td><button onclick='deleteEmp(" +
          i +
...
function getNextImg() {
  ...
  overlayInner.style.backgroundImage =
    "url(" + imgsArray[currentIndex].src + ")";
}
